...
/**
 * Generates the partnership proposal PDF via Browserless.io.
 *
 * Flow:
 * 1. Build the absolute /proposal-print/[id] URL
...
export async function GET(
  ...
  try {
    ...
    const filename = req.nextUrl.searchParams.get("filename") || `GymGaze-Proposal-${id}.pdf`;

    const printUrl = new URL(`/proposal-print/${id}`, origin);
    ...
    if (!browserlessRes.ok) {
      ...
      console.error("[proposal-pdf] Browserless error:", browserlessRes.status, errText);
      return NextResponse.json(
